...
export const generateJavaScript = (node: ASTNode): string => {
    ...
    switch (node.type) {
        case 'Program':
            ...
            return node.children?.map(child => generateJavaScript(child)).join('\n') || '';
        ...
        case 'VariableDeclaration': {
            ...
            const identifier = node.children?.find(child => child.type === 'Identifier')?.value;
        ...
        case 'Assignment': {
            // Variable assignment
            const identifier = node.children?.find(child => child.type === 'Identifier')?.value;
            const expression = node.children?.find(child => child.type === 'Expression');
        ...
        case 'IfElse': {
            ...
            const ifCommands = node.children?.find(child => child.type === 'IfCommands');
            const elseCommands = node.children?.find(child => child.type === 'ElseCommands');

            const jsCondition = condition ? generateJavaScript(condition) : '';
            const jsIfCommands = ifCommands?.children?.map(child => generateJavaScript(child)).join('\n') || '';
            const jsElseCommands = elseCommands?.children?.map(child => generateJavaScript(child)).join('\n') || '';
        ...
        case 'DoWhileLoop': {
            ...
            const jsCommands = commands?.children?.map(child => generateJavaScript(child)).join('\n') || '';
            const jsCondition = condition ? generateJavaScript(condition) : '';
        ...
        case 'Condition': {
            ...
            const left = node.children?.[0] ? generateJavaScript(node.children[0]) : '';
            const operator = node.children?.[1]?.value || '';
            const right = node.children?.[2] ? generateJavaScript(node.children[2]) : '';
